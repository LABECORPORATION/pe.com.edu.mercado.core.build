package pe.com.builderp.core.facturacion.ejb.dao.empresa.local;

import java.util.List;

import javax.ejb.Local;

import pe.com.builderp.core.facturacion.model.dto.empresa.ActividadDTO;
import pe.com.edu.siaa.core.ejb.dao.generic.local.GenericDAOLocal;
import pe.com.edu.siaa.core.model.jpa.empresa.Actividad;

/**
 * La Class ActividadDaoLocal.
 * <ul>
 * <li>Copyright 2017 ndavilal -
 * ndavilal. Todos los derechos reservados.</li>
 * </ul>
 *
 * @author ndavilal
 * @version 2.1, Thu Dec 28 12:02:21 COT 2017
 * @since SIAA-CORE 2.1
 */
@Local
public interface ActividadDaoLocal  extends GenericDAOLocal<String,Actividad> {
	/**
	 * Listar Actividad.
	 *
	 * @param Actividad el ActividadDTO
	 * @return the list
	 * @throws Exception the exception
	 */
	List<Actividad> listarActividad(ActividadDTO Actividad) throws Exception;
	
	/**
	 * contar lista Actividad.
	 *
	 * @param Actividad el Actividad
	 * @return the list
	 * @throws Exception the exception
	 */
	int contarListarActividad(ActividadDTO Actividad);
	/**
	 * Generar id Actividad.
	 *
	 * @return the Long
	 * @throws Exception the exception
	 */
	String generarIdActividad() throws Exception;
}